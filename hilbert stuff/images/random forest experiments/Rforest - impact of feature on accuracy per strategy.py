...
##        X_test, y_test = X.iloc[test_idx.flatten(),:], yf.iloc[test_idx.flatten()]
        ...
        for feature in features:
            ...
            rfcls.fit(X_train_drop,y_train)
            result[feature] = accuracy(y_test, rfcls.predict(X_test_drop))
        results.append(result)
rdf = pd.DataFrame(results)
rdf.to_csv('Feature impact on accuracy (fgrouping, drop).csv',index=False)
